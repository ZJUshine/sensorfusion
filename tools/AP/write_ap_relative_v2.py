# generates the comprehensive data of each model
import numpy as np
...
for model in model_type:
    for attack in attack_type:
            ...
            for c in class_type:
                for d in detection_type:
                    ...
                    if os.path.exists(RP_file):
                        ...
                        for i in range(3):
                            d1[:, i] = RP_data[:, (i+1)*3+1]  # 4,7,10 easy
                            d2[:, i] = RP_data[:, (i+1)*3+2]  # 5
                            d3[:, i] = RP_data[:, (i+1)*3+3]  # 6
        ...
        if attack == 'benign':
            ...
            content2 = [model, attack, 'ap_fp_fn_easy', AP_easy, 0,0, d1[-1][1], 0,0, d1[-1][2],0, 0]
            content3 = [model, attack, 'ap_fp_fn_moderate', AP_moderate, 0,0, d2[-1][1], 0,0, d2[-1][2],0, 0]            
            content4 = [model, attack, 'ap_fp_fn_hard',AP_hard,0, 0, d3[-1][1], 0,0, d3[-1][2],0, 0]
        else:
            ...
            content2 = [model, attack, 'tp_fp_fn_easy', AP_easy, AP_easy-benign_data[0],AP_easy/benign_data[0]-1, d1[-1][1], d1[-1][1]-benign_data[4],d1[-1][1]/benign_data[4]-1, d1[-1][2], d1[-1][2]-benign_data[5],d1[-1][2]/benign_data[5]-1]
            content3 = [model, attack, 'tp_fp_fn_moderate', AP_moderate, AP_moderate-benign_data[1],AP_moderate/benign_data[1]-1, d2[-1][1], d2[-1][1]-benign_data[7],d2[-1][1]/benign_data[7]-1, d2[-1][2], d2[-1][2]-benign_data[8],d2[-1][2]/benign_data[8]-1]            
            content4 = [model, attack, 'tp_fp_fn_hard', AP_hard, AP_hard-benign_data[2],AP_hard/benign_data[2]-1, d3[-1][1], d3[-1][1]-benign_data[10],d3[-1][1]/benign_data[10]-1, d3[-1][2], d3[-1][2]-benign_data[11],d3[-1][2]/benign_data[11]-1]
