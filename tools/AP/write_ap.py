...
result_csv = open("AP_train.csv",'w')
...
for model in model_type:
    for attack in attack_type:
            ...
            for c in class_type:
                for d in detection_type:
                    ...
                    if os.path.exists(RP_file):
                        ...
                        for i in range(3):
                            d1[:, i] = RP_data[:, (i+1)*3+1]
                            d2[:, i] = RP_data[:, (i+1)*3+2]
                            d3[:, i] = RP_data[:, (i+1)*3+3]
        ...
        if attack == 'benign':
            ...
            content1 = [model, attack, 'car_detection_3d', AP_easy, 0, AP_moderate, 0, AP_hard, 0]
            content2 = [model, attack, 'tp_fp_fn_easy', d1[-1][0], 0, d1[-1][1], 0, d1[-1][2], 0]
            content3 = [model, attack, 'tp_fp_fn_moderate', d2[-1][0], 0, d2[-1][1], 0, d2[-1][2], 0]            
            content4 = [model, attack, 'tp_fp_fn_hard', d3[-1][0], 0, d3[-1][1], 0, d3[-1][2], 0]
        else:
            content1 = [model, attack, 'car_detection_3d', AP_easy, AP_easy-benign_data[0], AP_moderate, AP_moderate-benign_data[1], AP_hard, AP_hard-benign_data[2]]
            content2 = [model, attack, 'tp_fp_fn_easy', d1[-1][0], d1[-1][0]-benign_data[3], d1[-1][1], d1[-1][1]-benign_data[4], d1[-1][2], d1[-1][2]-benign_data[5]]
            content3 = [model, attack, 'tp_fp_fn_moderate', d2[-1][0], d2[-1][0]-benign_data[6], d2[-1][1], d2[-1][1]-benign_data[7], d2[-1][2], d2[-1][2]-benign_data[8]]            
            content4 = [model, attack, 'tp_fp_fn_hard', d3[-1][0], d3[-1][0]-benign_data[9], d3[-1][1], d3[-1][1]-benign_data[10], d3[-1][2], d3[-1][2]-benign_data[11]]
